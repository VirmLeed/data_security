...
def main(text: str, alphabet: str, shift: int, keyword: str, out: str = None, debug: bool = False) -> None:
    ...
    result = caesar_cypher.decrypt(keyword_cypher.decrypt(text, keyword, debug), alphabet, shift, debug)
    # result = keyword_cypher.decrypt(caesar_cypher.decrypt(text, alphabet, shift, debug), keyword, debug)
